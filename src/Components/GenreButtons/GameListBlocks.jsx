...
export default function GameListBlocks() {
  ...
  const [regularGenres, setRegularGenres] = useState([]);
  ...
  useEffect(() => {
    fetch('/gameLibrary/games.json')
      ...
      .then(data => {
        ...
        const genreImagesMap = data.games.reduce((acc, game) => {
        ...
        const genresList = Object.keys(genreImagesMap);
        const specialGenresList = genresList.filter(genre => specialTags.includes(genre));
        const regularGenresList = genresList.filter(genre => !specialTags.includes(genre));
        
        setGenreImages(genreImagesMap);
        setGenres(genresList);
        setSpecialGenres(specialGenresList);
        setRegularGenres(regularGenresList);
        
        // Initialize current indexes
        const initial = {};
  ...
  if (!gamesData) {
    return <div className="min-h-screen flex items-center justify-center text-white">Loading genres...</div>;
  }
